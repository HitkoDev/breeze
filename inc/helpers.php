...
function breeze_validate_the_right_extension($url_list = [], $extension = 'css') {
    ...
    foreach ($url_list as $url) {
        ...
        if ($is_regexp === false) {
            ...
            if ($file_extension !== false && strtolower($extension) !== $file_extension) {
                $is_valid = false;
            }
        }
 ...
 * @since 1.1.0
 */
function breeze_is_string_in_array_values($needle = '', $haystack = []) {
 ...
 * @since 1.1.7
 */
function breeze_check_for_exclude_values($needle = '', $haystack = []) {
    if (empty($needle) || empty($haystack)) {
        return [];
    }
    $needle = trim($needle);
...
function breeze_get_headers_via_curl($url_ping = '') {
    ...
    if (isset($headers['x-cacheable'])) {
        ...
        if (strtolower($x_cacheable_value) === 'yes' || strtolower($x_cacheable_value) === 'short') {
            return true;
        }
    }
...
function is_varnish_layer_started() {
    ...
    if (isset($data['HTTP_X_VARNISH'], $data['HTTP_X_APPLICATION'])) {
        if (trim($data['HTTP_X_APPLICATION']) === 'varnishpass') {
            return false;
        }
        if (trim($data['HTTP_X_APPLICATION']) === 'bypass') {
            return false;
        }
        if (is_null($data['HTTP_X_APPLICATION'])) {
            return false;
        }
    }
