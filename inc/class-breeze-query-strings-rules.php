...
class Breeze_Query_Strings_Rules {
    ...
    public static function when_woocommerce_settings_save() {
        if (isset($_POST['save'], $_POST['woocommerce_default_customer_address'])) {
    ...
    public function fetch_always_cache_list() {
        ...
        if (isset($GLOBALS['breeze_config']['woocommerce_geolocation_ajax_inherit']) && !empty($GLOBALS['breeze_config']['woocommerce_geolocation_ajax_inherit'])) {
            ...
            if (
                ...
                && (int) $GLOBALS['breeze_config']['woocommerce_geolocation_ajax_inherit']['subsite_' . $sub_blog_id] === 1
            ) {
                $this->always_cache_query[] = 'v';
            }
        } elseif (isset($GLOBALS['breeze_config']['woocommerce_geolocation_ajax']) && (int) $GLOBALS['breeze_config']['woocommerce_geolocation_ajax'] === 1) {
            $this->always_cache_query[] = 'v';
        }

        if (isset($GLOBALS['breeze_config'], $GLOBALS['breeze_config']['permalink_structure'])) {
     ...
     * @since 1.2.4
     */
    public function breeze_is_query_string_ignore() {
    ...
    public function check_query_var_group($current_url = '') {
        ...
        $found_items = [
            ...
            'ignored_items' => [], // which items were found in ignore list.
            'cached_no' => 0, // query vars that need to be cached.
            'cached_items' => [], // which items were found in to cache list.
            'user_cached_no' => 0, // query vars that need to be cached, these are defined by the user in back-end.
            'user_cached_items' => [], // which items were found in to cache list, these are defined by the user in back-end.
            'extra_query_no' => 0, // number of query vars not found in any list.
            'extra_query_vars' => [], // list of query vars not found in any list.
        ];
