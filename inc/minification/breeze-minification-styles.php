...
class Breeze_MinificationStyles extends Breeze_MinificationBase {
    ...
    private $css_group_val = [];
    private $css_min_arr = [];
    private $issetminfile = false;
    private $url_group_arr = [];
                    ...
                    if (preg_match('#<link.*href=("|\')(.*)("|\')#Usmi', $tag, $source)) {
                        ...
                        if ($this->include_inline) {
                            ...
                            if ($this->group_css == true) {
                                if (isset($media[0]) && trim($media[0]) === 'print') {
                                    if (strpos($code, '@media') === false) {
                                        $code = '@media print{' . $code . '}';
                                    }
                                } elseif (isset($media[0]) && trim($media[0]) === 'speech') {
                                    if (strpos($code, '@media') === false) {
                                        $code = '@media speech{' . $code . '}';
                                    }
                                } elseif (isset($media[0]) && trim($media[0]) === 'screen') {
                                    if (strpos($code, '@media') === false) {
                ...
                while (preg_match_all('#^(/*\s?)@import.*(?:;|$)#Um', $thiscss, $matches)) {
                    foreach ($matches[0] as $import) {
                        if ($this->isremovable($import, $this->cssremovables)) {
                            ...
                            if ($this->include_imported_css === true && file_exists($path) && is_readable($path)) { // add settings for this
            ...
            foreach ($this->csscode as &$code) {
                ...
                if (($this->alreadyminified !== true) && (apply_filters('breeze_css_do_minify', true))) {
                    if (class_exists('Minify_CSS_Compressor')) {
                        ...
                        } elseif (@is_callable([$cssmin, 'minify'])) {
        ...
        if ($this->group_css == true) {
            if ($this->inline == true) {
                ...
                if ($this->defer == true) {
                    ...
                    $this->inject_in_html($deferredCssBlock, ['</body>', 'before']);
                }
            }
        } else {
            if ($this->inline == true) {
    ...
    private function can_inject_late($cssPath, $css) {
        ...
        if ((strpos($css, '@font-face') !== false) && (apply_filters('breeze_filter_css_fonts_cdn', false) === true) && (!empty($this->cdn_url))) {
            // don't late-inject CSS with font-src's if fonts are set to be CDN'ed
            return false;
        }
        if ((($this->datauris == true) || (!empty($this->cdn_url))) && preg_match('#background[^;}]*url\(#Ui', $css)) {
