...
class Breeze_Settings_Import_Export {
    public function __construct() {
        ...
        add_action('wp_ajax_breeze_export_json', [&$this, 'export_json_settings']);
        add_action('wp_ajax_breeze_import_json', [&$this, 'import_json_settings']);
    }
    ...
    public function import_json_settings() {
        ...
        if (isset($_FILES['breeze_import_file'])) {
            ...
            if ($_FILES['breeze_import_file']['type'] !== 'application/json') {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                if (
                    isset($json['breeze_basic_settings'], $json['breeze_advanced_settings'], $json['breeze_cdn_integration'])
                ) {
                    $level = '';
                    if (is_multisite()) {
     ...
     * @since 1.2.2
     */
    public function replace_options($options = [], $level = '') {
        if (empty($options)) {
            return false;
        }
        ...
        if (is_multisite()) {
            ...
            if ($level === 'network') {
                foreach ($options as $meta_key => $meta_value) {
                    if (strpos($meta_key, 'breeze_') !== false) {
     ...
     * @since 1.2.2
     */
    public static function replace_options_cli($options = [], $level = '') {
        if (empty($options)) {
            return false;
        }
