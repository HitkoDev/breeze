...
class Breeze_Configuration {
    public function __construct() {
        ...
        add_action('load-settings_page_breeze', [$this, 'afterLoadConfigPage']);
    }
    ...
    public function afterLoadConfigPage() {
        ...
        if (
            is_multisite()
            && isset($_REQUEST['inherit-settings'], $_POST['breeze_inherit_settings_nonce'])
            && wp_verify_nonce($_POST['breeze_inherit_settings_nonce'], 'breeze_inherit_settings')
        ) {
            $inherit_settings = (filter_var($_REQUEST['inherit-settings'], FILTER_VALIDATE_BOOLEAN) === true ? '1' : '0');
            ...
            if (!is_network_admin() && $is_inherit_settings_change === true) {
            ...
            //return current page
            if (!is_network_admin() && $is_inherit_settings_change === true && !empty($_REQUEST['_wp_http_referer'])) {
        ...
        if (isset($_REQUEST['breeze_basic_action']) && $_REQUEST['breeze_basic_action'] == 'breeze_basic_settings') {
            if (isset($_POST['breeze_settings_basic_nonce']) && wp_verify_nonce($_POST['breeze_settings_basic_nonce'], 'breeze_settings_basic')) {
                ...
                if (!is_network_admin() && $is_inherit_settings_on === true && !empty($_REQUEST['_wp_http_referer'])) {
                    $url = remove_query_arg('database-cleanup', $_REQUEST['_wp_http_referer']);
                    wp_safe_redirect(add_query_arg('save-settings', 'success', $url));
                    exit;
                }

                $post_activate_cache = isset($_POST['breeze-admin-cache']) ? $_POST['breeze-admin-cache'] : [];
    ...
    public static function update_htaccess($clean = false) {
        ...
        if (is_multisite()) {
            ...
            $browser_cache_sites = [];
            $no_browser_cache_sites = [];
            $browser_cache_regex = '';
            $has_gzip_compress = false;
            $gzip_compress_sites = [];
            $no_gzip_compress_sites = [];
            $gzip_compress_regex = '';
            ...
            foreach ($blogs as $blog_id) {
                ...
                if ($config['breeze-active'] === '1') {
                    if ($config['breeze-browser-cache'] === '1') {
                        $has_browser_cache = true;
                        $browser_cache_sites[] = $site_url;
                    } else {
                        $no_browser_cache_sites[] = $site_url;
                    }
                    if ($config['breeze-gzip-compression'] === '1') {
            ...
            foreach ($rules as $var_name => $method_name) {
                ...
                if (!${$has_cache_var}) {
                    ...
                    if (defined('SUBDOMAIN_INSTALL') && SUBDOMAIN_INSTALL) {
                        ...
                        if ($network_site_url_index !== false) {
            ...
            $config = breeze_get_option('basic_settings');
            if ($config['breeze-active'] === '1') {
                self::add_expires_header($config['breeze-browser-cache'] !== '1');
                self::add_gzip_htacess($config['breeze-gzip-compression'] !== '1');
            } else {
    ...
    */
    ...
    public static function breeze_ajax_purge_database() {
        ...
        $type = ['revisions', 'drafted', 'trash', 'comments', 'trackbacks', 'transient'];
