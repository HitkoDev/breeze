...
if ( ! class_exists('Breeze_Incompatibility_Plugins')) {
    class Breeze_Incompatibility_Plugins {
        ...
        public function __construct() {
            ...
            add_action('admin_notices', [&$this, 'prepare_and_display_notification_content'], 15);
            add_action('network_admin_notices', [&$this, 'prepare_and_display_notification_content'], 15);

            add_action('wp_ajax_compatibility_warning_close', [&$this, 'compatibility_warning_close']);
        }
                ...
                foreach ($incompatible_plugins as $plugin => $details) {
                    if (isset($installed_plugins[$plugin])) {
                        ...
                        if (is_plugin_active($plugin)) {
                            ...
                            } elseif ($warning_version_int === -1) { // incompatible no matter the the version
                                $is_warning = true;
                            }
                            ...
                            if ($is_warning === true) {
                                ...
                                if ($current_screen->base === 'plugins-network') {
                                    if (current_user_can('manage_network_plugins')) {
                                        $show_deactivate = true;
                                    }
                                } elseif ($current_screen->base === 'plugins') {
