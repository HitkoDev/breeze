...
class Breeze_PurgeVarnish {
    ...
    protected $actionsNoId = ['switch_theme'];
            ...
            if (!empty($this->actions)) {
                ...
                foreach ($this->actions as $action) {
                    if (in_array($action, $this->actionsNoId)) {
                        ...
                        add_action($action, [$this, 'purge_post']);
                    }
                }
            }
            //Pust urlsPurge after comment
            add_action('comment_post', [$this, 'purge_post_on_comment'], 10, 3);
            add_action('wp_set_comment_status', [$this, 'purge_post_on_comment_status'], 10, 2);
        }
        //Execute Purge
        add_action('shutdown', [$this, 'breeze_execute_purge']);
    }
    ...
    public function purge_post($postId) {
        if (current_action() === 'save_post' && did_action('edit_post')) {
