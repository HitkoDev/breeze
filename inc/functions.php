 ...
 * @return int
 */
function breeze_get_directory_size($dir, $exclude = []) {
    $size = 0;
...
function breeze_uri_amp_check($url = '') {
    if (
        strpos($url, '/amp/') !== false
        || strpos($url, 'amp=1') !== false
        || strpos($url, '?amp') !== false
    ) {
        return true;
    }
...
function breeze_which_role_folder($hash = '') {
    ...
    if (isset($GLOBALS['breeze_config'], $GLOBALS['breeze_config']['wp-user-roles'])) {
