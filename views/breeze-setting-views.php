...
<div id="message-save-settings"
    class="notice notice-success is-dismissible"
    style="margin: 10px 0px 10px 0;padding: 10px;"><strong><?php _e('Database cleanup successful', 'breeze'); ?></strong></div>
<?php } ?>
<!--save settings successfull message-->
<?php if (isset($_REQUEST['save-settings']) && $_REQUEST['save-settings'] == 'success') { ?>
<div id="message-save-settings"
    class="notice notice-success is-dismissible"
    style="margin: 10px 0px 10px 0;padding: 10px;"><strong><?php _e('Configuration settings saved', 'breeze'); ?></strong></div>
<?php } ?>
<div class="wrap breeze-main">
    <div class="breeze-header">
        <a href="https://www.cloudways.com"
            target="_blank">
    ...
    if ($is_subsite) {
        ...
        if ($inherit_settings != 0) {
            $inherit_settings = 1;
            $show_tabs = false;
        } ?>
    <form id="breeze-inherit-settings-toggle"
        class="breeze-form"
        method="post"
        action="">
        <div class="radio-field<?php echo $inherit_settings == 1 ? ' active' : ''; ?>">
            <label>
                <input type="radio"
                    ...
                    value="1"
                    <?php checked($inherit_settings, 1); ?>>
        ...
        <div class="radio-field<?php echo $inherit_settings == 0 ? ' active' : ''; ?>">
            <label>
                <input type="radio"
                    ...
                    name="inherit-settings"
                    value="0"
                    <?php checked($inherit_settings, 0); ?>>
        ...
        <p class="disclaimer"><?php esc_html_e('To apply your changes, please click on the Save Changes button.', 'breeze'); ?>
        </p>
    ...
    <ul id="breeze-tabs"
        class="nav-tab-wrapper <?php echo !$show_tabs ? 'tabs-hidden' : ''; ?>">
        <?php
    ...
    <div id="breeze-tabs-content"
        class="tab-content <?php echo !$show_tabs ? 'tabs-hidden' : ''; ?>">
        <?php
        foreach ($tabs as $key => $name) {
            $is_inactive = !$show_tabs && !in_array($key, $global_tabs);
        ...
        <!--Right-side content-->
        <div id="breeze-and-cloudways"
            class="rs-block">
            <h3 class="rs-title"><?php _e('Want to Experience Better Performance?', 'breeze'); ?>
            </h3>
            <div class="rs-content">
                <p><?php _e('Take advantage of powerful features by deploying WordPress and Breeze on Cloudways.', 'breeze'); ?>
                </p>
                <ul>
                    <li><?php _e('Fully Compatible with Varnish', 'breeze'); ?>
                    </li>
                    <li><?php _e('One-Click setup of CloudwaysCDN', 'breeze'); ?>
                    </li>
                    <li><?php _e('24/7 Expert Human Support', 'breeze'); ?>
                    </li>
                    <li><?php _e('WooCommerce Compatible', 'breeze'); ?>
                    </li>
                </ul>
                <button class="button button-primary">
                    <a href="https://www.cloudways.com/en/wordpress-cloud-hosting.php?utm_source=breeze-plugin&utm_medium=breeze&utm_campaign=breeze"
                        target="_blank"><?php _e('Find Out More', 'breeze'); ?></a>
                </button>
            </div>
            <div class="rs-content">
                <h4><?php _e('Rate Breeze', 'breeze'); ?>
                </h4>
                <p><?php _e('If you are satisfied with Breeze\'s performance, <a href="https://wordpress.org/plugins/breeze#reviews" target="_blank">drop us a rating here.</a>', 'breeze'); ?>
                </p>
            </div>
        </div>
    </div>
